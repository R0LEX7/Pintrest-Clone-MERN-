...
const FeedCard = ({ props }) => {
  ...
  useEffect(() => {
    socket.on("isLiked", ({ postId, likeArray }) => {
      if (postId === props._id) {
  ...
  }, [socket, user, props._id]);

  const handleClick = async () => {
    try {
      if (user) {
        const updatedLikeArray = await handleLike(props._id);
        setLikeArr(updatedLikeArray);
        socket.emit("like", { postId: props._id, likeArray: updatedLikeArray });
        socket.on("isLiked", ({ postId, likeArray }) => {
          if (postId === props._id) {
            setLikeArr(likeArray);
          }
